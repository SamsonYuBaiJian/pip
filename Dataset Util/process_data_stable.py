...
def save_data(data, start_idx, end_idx, label_path, task_type):
    ...
    for i in range(start_idx, end_idx):
        ...
        if len(unused_obj) > 0:
            # print(len(unused_obj))
            print(obj_color_dict.keys())
            print(unused_obj[0][0])
            print(unused_obj[0][1])
            print(len(obj_color_dict.keys()))
            if len(unused_obj) == 1 and len(obj_color_dict.keys()) == 1:
                output_data[sample_num].append([unused_obj[0][0], obj_color_dict['Torus'], unused_obj[0][1],str(i+1)])
...
def get_dataset_splits(data_path, video_path, train_val_test_splits, task_type, train_label_path, val_label_path, test_label_path):
    ...
    # save splits
    # save_data(data, 401, 600, train_label_path, task_type)
    # save_data(data, 734, 800, val_label_path, task_type)
    save_data(data, 0, 200, test_label_path, task_type)
