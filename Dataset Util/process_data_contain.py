    ...
    for i in range(start_idx, end_idx):
        ...
        if len(unused_obj) > 0:
            ...
            if len(unused_obj) == 1 and len(obj_color_dict.keys()) == 1:
                output_data[sample_num].append([unused_obj[0][0], obj_color_dict['Torus.020'], unused_obj[0][1],str(i+1)])
...
def get_dataset_splits(data_path, video_path, train_val_test_splits, task_type, train_label_path, val_label_path, test_label_path):
    ...
    # save splits
    # print(train_num)
    # print(int(train_num + val_num))
    # print(int(data_num))
    # save_data(data, 200, 400, train_label_path, task_type)
    # save_data(data, 667, 733, val_label_path, task_type)
