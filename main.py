...
def train(cfg, task_type, frame_path, mask_path, train_label_path, val_label_path, test_label_path, save_path, num_epoch, batch_size, teacher_forcing_prob, first_n_frame_dynamics, frame_interval, learning_rate, save_frames_every, max_seq_len, span_num, span_threshold):
    # get experiment ID
    ...
    train_dataset = Data(frame_path, mask_path, train_label_path, frame_interval, first_n_frame_dynamics)
    train_dataloader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True, num_workers=0)
    val_dataset = Data(frame_path, mask_path, val_label_path, frame_interval, first_n_frame_dynamics)
    ...
    model = Model(device, 3, 16, span_num).to(device)
    ...
    for i in range(num_epoch):
        ...
        for j, batch in tqdm(enumerate(train_dataloader)):
            ...
            pred_labels, pred_images_seq, all_r, jsd_loss = model(task_type, frames, masks, teacher_forcing_batch, first_n_frame_dynamics, max_seq_len)
        ...
        # print("Epoch {}/{} OVERALL train gen loss={}.".format(i+1, num_epoch, sum(temp_train_image_loss) / total_cnt))
        ...
        # stats['train']['mask_loss'].append(sum(temp_train_mask_loss) / total_cnt)
        ...
        # print("Epoch {}/{} OVERALL validation gen loss={}.".format(i+1, num_epoch, sum(temp_val_image_loss) / total_cnt))
...
if __name__ == '__main__':
    ...
    if experiment_type == 'train':
        train(cfg, task_type, frame_path, mask_path, train_label_path, val_label_path, test_label_path, save_path, num_epoch, batch_size, teacher_forcing_prob, first_n_frame_dynamics, frame_interval, learning_rate, save_frames_every, max_seq_len, span_num, span_threshold)
